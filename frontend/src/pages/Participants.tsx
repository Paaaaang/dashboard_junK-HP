...
function CompletionBadge({ status }: CompletionBadgeProps) {
  ...
  return (
    ...
    >
      {isCompleted ? <CheckCircle2 style={{ width: 12, height: 12 }} aria-hidden="true" /> : <Circle style={{ width: 12, height: 12 }} aria-hidden="true" />} {status}
    </span>
  );
}
...
function CompanyPopover({ participant, onNavigate, allParticipants }: CompanyPopoverProps) {
    ...
    >
      <div>
        ...
        <div style={{ fontSize: 12, color: "var(--color-text-tertiary)" }}>
          {participant.companyLocation} · 대표 {participant.companyRepresentative}
        </div>
      </div>
      <hr style={{ margin: 0, border: 0, borderTop: "1px solid var(--color-border)" }} />
      <div style={{ display: "grid", gap: 4 }}>
        {participant.companyManager && (
          <div style={{ fontSize: 12, display: "flex", gap: 8 }}>
            <span style={{ color: "var(--color-text-tertiary)", width: 44 }}>담당자</span>
            <span style={{ color: "var(--color-text-primary)", fontWeight: 500 }}>{participant.companyManager}</span>
          </div>
        )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>
          협약 {participant.mouSigned ? "✅ 체결" : "❌ 미체결"} · 참여자 {participantCount}명
        </span>
        <button
          type="button"
...
interface EnrollmentRowProps {
  ...
  onStatusChange: (enrollmentId: string, status: CompletionStatus, completionDate?: string, certificateNo?: string) => void;
}
function EnrollmentRow({ enrollment, onStatusChange }: EnrollmentRowProps) {
  const [expanded, setExpanded] = useState(false);
  const [changingStatus, setChangingStatus] = useState(false);
  const [newCompletionDate, setNewCompletionDate] = useState(enrollment.completionDate ?? "");
  const [newCertificateNo, setNewCertificateNo] = useState(enrollment.certificateNo ?? "");
  ...
  const handleSave = () => {
    if (pendingStatus === "수료") {
      onStatusChange(enrollment.id, "수료", newCompletionDate || undefined, newCertificateNo || undefined);
    }
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 0 }}>
          <span style={{ color: COURSE_TYPE_COLOR[enrollment.courseType], flexShrink: 0 }}>
            {expanded ? "▾" : "▸"}
          </span>
          <span
            style={{
      ...
      {expanded && (
        ...
        >
          <div style={{ display: "grid", gridTemplateColumns: "100px 1fr", gap: "6px 12px" }}>
            <span style={{ color: "var(--color-text-tertiary)", fontSize: 12 }}>진행 기간</span>
            <span style={{ color: "var(--color-text-primary)" }}>{formatDateRange(enrollment.startDate, enrollment.endDate)}</span>

            <span style={{ color: "var(--color-text-tertiary)", fontSize: 12 }}>교육 시간</span>
            <span style={{ color: "var(--color-text-primary)" }}>{enrollment.totalHours}시간</span>

            <span style={{ color: "var(--color-text-tertiary)", fontSize: 12 }}>수료 상태</span>
            ...
            {enrollment.completionDate && (
              <>
                <span style={{ color: "var(--color-text-tertiary)", fontSize: 12 }}>수료일</span>
                <span style={{ color: "var(--color-text-primary)" }}>{enrollment.completionDate}</span>
              </>
            )}

            {enrollment.certificateNo && (
              <>
                <span style={{ color: "var(--color-text-tertiary)", fontSize: 12 }}>수료번호</span>
                <span style={{ color: "var(--color-text-primary)", fontFamily: "monospace" }}>{enrollment.certificateNo}</span>
              </>
            )}

            {enrollment.applicationDate && (
              <>
                <span style={{ color: "var(--color-text-tertiary)", fontSize: 12 }}>신청일</span>
                <span style={{ color: "var(--color-text-primary)" }}>{enrollment.applicationDate}</span>
              </>
            )}
          </div>
            ...
            >
              ...
              <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                <button
                  ...
                  onClick={() => { setChangingStatus(false); setPendingStatus(""); }}
                >
                  취소
                </button>
                <button
...
function AddParticipantModal({ onClose, onAdd, allCompanies }: AddParticipantModalProps) {
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof AddParticipantForm, string>>>({});
  ...
  const handleSubmit = () => {
    if (!validate()) return;
    const company = allCompanies.find((c: CompanyRecord) => c.id === form.companyId);
  ...
  const set = <K extends keyof AddParticipantForm>(key: K, value: AddParticipantForm[K]) => {
  ...
  return (
    <div className="modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-panel modal-panel-sm" style={{ width: "min(560px, 100%)" }}>
        ...
        <div className="modal-content">
          <div className="form-grid">
            {/* 이름 */}
            <div className="field">
              <label>이름 <span style={{ color: "var(--color-error)" }}>*</span></label>
              <input
                id="add-name"
                type="text"
                value={form.name}
              ...
              />
              {errors.name && <p id="err-name" role="alert" className="form-error-message" style={{ padding: "4px 8px", marginTop: 0 }}>{errors.name}</p>}
            </div>
            ...
            <div className="field form-full" style={{ position: "relative" }}>
              <label>소속 기업 <span style={{ color: "var(--color-error)" }}>*</span></label>
              <input
                type="text"
                ...
                placeholder="기업명 검색..."
              />
              {errors.companyName && <p className="form-error-message" style={{ padding: "4px 8px", marginTop: 0 }}>{errors.companyName}</p>}
              {showDropdown && (
                <div className="chip-dropdown" style={{ top: "calc(100% + 4px)" }}>
            ...
            {/* 연락처 */}
            <div className="field">
              <label>연락처 <span style={{ color: "var(--color-error)" }}>*</span></label>
              <input
                id="add-phone"
                type="tel"
                value={form.phone}
                ...
                style={{ borderColor: errors.phone ? "var(--color-error)" : undefined }}
              />
              {errors.phone && <p id="err-phone" role="alert" className="form-error-message" style={{ padding: "4px 8px", marginTop: 0 }}>{errors.phone}</p>}
            </div>

            {/* 이메일 */}
            <div className="field">
              <label>이메일 <span style={{ color: "var(--color-error)" }}>*</span></label>
              <input
                type="email"
                value={form.email}
              ...
              />
              {errors.email && <p className="form-error-message" style={{ padding: "4px 8px", marginTop: 0 }}>{errors.email}</p>}
            </div>
            ...
            <div className="field">
              ...
              <select
                ...
                onChange={(e) => set("workExperience", e.target.value as WorkExperience | "")}
              >
            ...
            <div className="field form-full">
              ...
              <select
                className="select-field"
                style={{ height: 40, padding: "0 12px", borderRadius: 10, width: "100%" }}
                value={form.documentSkill}
                onChange={(e) => set("documentSkill", e.target.value as DocumentSkill | "")}
              >
                <option value="">선택 안 함</option>
        ...
        <div className="modal-footer">
          ...
          <button type="button" className="btn btn-primary" onClick={handleSubmit}>추가</button>
        </div>
      </div>
    </div>
  );
...
const COURSE_CATALOG: Record<CourseType, Array<{ name: string; startDate: string; endDate: string; hours: number }>> = {
  훈련비과정: [
    { name: "스마트팩토리 실무", startDate: "2026-04-14", endDate: "2026-04-16", hours: 16 },
    { name: "고급 CAD 실습", startDate: "2026-05-06", endDate: "2026-05-08", hours: 20 },
    { name: "IoT 기초", startDate: "2026-05-18", endDate: "2026-05-19", hours: 12 },
    { name: "의료기기 품질관리 기초 및 실무", startDate: "2026-05-12", endDate: "2026-05-14", hours: 24 },
  ],
  지원비과정: [
    { name: "품질관리 고도화", startDate: "2026-04-21", endDate: "2026-04-23", hours: 20 },
    { name: "생산성 향상", startDate: "2026-05-07", endDate: "2026-05-08", hours: 12 },
    { name: "ERP 활용", startDate: "2026-05-20", endDate: "2026-05-22", hours: 18 },
  ],
  "공유개방 세미나": [
    { name: "AI 현장 적용 세미나", startDate: "2026-04-05", endDate: "2026-04-05", hours: 8 },
    { name: "스마트제조 트렌드", startDate: "2026-04-19", endDate: "2026-04-19", hours: 8 },
    { name: "디지털 전환 전략", startDate: "2026-05-10", endDate: "2026-05-10", hours: 8 },
  ],
};
...
function LinkCourseModal({ participant, onClose, onLink }: LinkCourseModalProps) {
  const [selectedType, setSelectedType] = useState<CourseType>("훈련비과정");
  const [selectedCourse, setSelectedCourse] = useState("");
  const [applicationDate, setApplicationDate] = useState("");

  const enrolledNames = new Set(participant.enrollments.map((e) => e.subCourseName));
  ...
  const handleLink = () => {
    ...
    const course = COURSE_CATALOG[selectedType].find((c) => c.name === selectedCourse);
    if (!course) return;
  ...
  return (
    <div className="modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-panel modal-panel-sm" style={{ width: "min(480px, 100%)" }}>
        <div className="modal-header">
        ...
        <div className="modal-content">
          ...
          {selectedCourse && (
            <p style={{ margin: 0, fontSize: 12, color: "var(--color-text-tertiary)", background: "var(--color-surface-subtle)", padding: "8px 12px", borderRadius: 8 }}>
...
function ParticipantDrawer({ participant, onClose, onUpdate, allCompanies, participants, isClosing }: DrawerProps) {
  ...
  const [expandedCourseTypes, setExpandedCourseTypes] = useState<Set<CourseType>>(
  ...
  const filteredCompanies = useMemo(
    () => allCompanies.filter((c: CompanyRecord) => c.companyName.toLowerCase().includes(companySearch.toLowerCase())),
    [companySearch, allCompanies],
  );
  ...
  const saveInfo = () => {
    setIsSaving(true);
    setTimeout(() => {
      const company = allCompanies.find((c: CompanyRecord) => c.id === draftInfo.companyId);
      onUpdate({
        ...participant,
        ...
        companyRepresentative: company?.representative ?? participant.companyRepresentative,
  ...
  const handleEnrollmentStatusChange = (
    ...
    const updated: ParticipantRecord = {
      ...
      enrollments: participant.enrollments.map((e) =>
        e.id === enrollmentId
          ? { ...e, status, completionDate: completionDate ?? e.completionDate, certificateNo: certificateNo ?? e.certificateNo }
          : e,
      ),
    };
    onUpdate(updated);
  ...
  const handleLinkCourse = (participantId: string, enrollment: ParticipantEnrollment) => {
    void participantId;
    onUpdate({ ...participant, enrollments: [...participant.enrollments, enrollment] });
        ...
        <div className="drawer-header">
          <div className="drawer-title-stack" style={{ flex: 1, minWidth: 0 }}>
            {editingName ? (
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === "Escape") { setDraftName(participant.name); setEditingName(false); }
                }}
                onBlur={commitName}
              />
            ) : (
            ...
            <p style={{ margin: 0, fontSize: 13, color: "var(--color-text-tertiary)" }}>
              <span
                style={{
                  color: participant.companyId ? "var(--color-info)" : "inherit",
              ...
              >
                ...
                {showCompanyPopover && participant.companyId && (
                  <CompanyPopover participant={participant} onNavigate={navigateToCompany} allParticipants={participants || []} />
                )}
              </span>
              {participant.position && ` · ${participant.position}`}
            </p>
          </div>
          <button type="button" className="icon-btn" onClick={onClose} aria-label="드로어 닫기">
          ...
          <div className="drawer-section">
            <div className="drawer-section-header">
              ...
              <div className="section-edit-actions">
                {editingInfo ? (
                  <>
                    <button type="button" className="section-icon-btn save" title="저장 (Enter)" onClick={saveInfo} disabled={isSaving}>
                      <Check size={13} />
                    </button>
                    <button type="button" className="section-icon-btn cancel" title="취소 (Esc)" onClick={cancelInfo}>
                      <X size={13} />
                    </button>
                  </>
                ) : (
                  <button type="button" className="section-icon-btn" title="편집" onClick={() => setEditingInfo(true)}>
                    <PencilLine size={13} />
                  </button>
                )}
              </div>
            ...
            <div className={`drawer-info-grid${editingInfo ? " edit-mode" : ""}`}>
              {/* 소속 기업 */}
                ...
                {editingInfo ? (
                  <div style={{ position: "relative" }}>
                    <input
                      ...
                      onChange={(e) => {
                        setCompanySearch(e.target.value);
                        setDraftInfo((p) => ({ ...p, companyName: e.target.value, companyId: "" }));
                      ...
                      <div className="chip-dropdown">
                        {filteredCompanies.map((c: CompanyRecord) => (
                          <button
                            ...
                            className="chip-option"
                            onMouseDown={() => {
                              setDraftInfo((p) => ({ ...p, companyId: c.id, companyName: c.companyName }));
                              setCompanySearch("");
                  ...
                  <strong
                    onClick={navigateToCompany}
                    style={{
                      color: participant.companyId ? "var(--color-info)" : "var(--color-text-primary)",
                      cursor: participant.companyId ? "pointer" : "default",
                      textDecoration: participant.companyId ? "underline" : "none",
                    }}
                  >
              ...
              <div className="drawer-info-item">
                ...
                {editingInfo ? (
                  <input
                    ...
                    onChange={(e) => setDraftInfo((p) => ({ ...p, position: e.target.value }))}
                    placeholder="직위"
                  />
                ) : (
              ...
              <div className="drawer-info-item">
                ...
                {editingInfo ? (
                  <input
                    ...
                    onChange={(e) => setDraftInfo((p) => ({ ...p, email: e.target.value }))}
                    placeholder="name@company.com"
                  />
                ) : (
                  <strong style={{ wordBreak: "break-all", fontSize: 12 }}>{participant.email || "—"}</strong>
                )}
              </div>
                ...
                {editingInfo ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, paddingTop: 4 }}>
                    <ToggleSwitch
                      ...
                      onChange={(v) => setDraftInfo((p) => ({ ...p, employmentInsurance: v ? "가입" : "미가입" }))}
                    />
                    <span style={{ fontSize: 12, color: "var(--color-text-secondary)", display: "flex", alignItems: "center", gap: "6px" }}>
                      {draftInfo.employmentInsurance === "가입" ? <><CheckCheck style={{ width: 14, height: 14, color: "#10b981" }} aria-hidden="true" /> 가입</> : <><HelpCircle style={{ width: 14, height: 14, color: "#94a3b8" }} aria-hidden="true" /> 미가입</>}
                    </span>
                  </div>
                ) : (
                  <strong style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    {participant.employmentInsurance === "가입" ? <><CheckCheck style={{ width: 14, height: 14, color: "#10b981" }} aria-hidden="true" /> 가입</> : participant.employmentInsurance === "미가입" ? <><HelpCircle style={{ width: 14, height: 14, color: "#94a3b8" }} aria-hidden="true" /> 미가입</> : <><HelpCircle style={{ width: 14, height: 14, color: "#94a3b8" }} aria-hidden="true" /> 미확인</>}
                  </strong>
                )}
              </div>
              ...
              <div className="drawer-info-item">
                ...
                {editingInfo ? (
                  <select
                    ...
                    onChange={(e) => setDraftInfo((p) => ({ ...p, workExperience: e.target.value as WorkExperience | "" }))}
                    style={{ padding: "3px 8px" }}
                  >
                    <option value="">미선택</option>
                    {WORK_EXPERIENCE_OPTIONS.map((o) => <option key={o} value={o}>{o}</option>)}
                  </select>
                ) : (
              ...
              <div className="drawer-info-item drawer-info-full">
                ...
                {editingInfo ? (
                  <select
                    ...
                    onChange={(e) => setDraftInfo((p) => ({ ...p, documentSkill: e.target.value as DocumentSkill | "" }))}
                    style={{ padding: "3px 8px" }}
                  >
                    <option value="">미선택</option>
                    {DOCUMENT_SKILL_OPTIONS.map((o) => <option key={o} value={o}>{o}</option>)}
                  </select>
                ) : (
            ...
            <div className="drawer-section-header">
              ...
              <span
                style={{
                  fontSize: 12,
                  fontWeight: 600,
                  background: completed === total && total > 0 ? "#ecfdf5" : "#f1f5f9",
                  color: completed === total && total > 0 ? "#065f46" : "#64748b",
                  padding: "2px 10px",
                  borderRadius: 999,
                }}
              >
              ...
              {COURSE_TYPES.map((ct) => {
                ...
                return (
                  <div key={ct} className="drawer-course-group">
                    ...
                    >
                      <span>{COURSE_TYPE_DOT[ct]} {ct}</span>
                      {isExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                    </button>
                    {isExpanded && (
                      <div className="drawer-course-group-body">
                        {enrollments.length === 0 ? (
                          <p className="drawer-course-empty">등록된 과정이 없습니다</p>
                        ) : (
                          enrollments.map((enr) => (
                            <EnrollmentRow
                              key={enr.id}
...
export function ParticipantsPage() {
  ...
  const openDrawer = useCallback((id: string) => {
    ...
    setSearchParams((prev) => { const next = new URLSearchParams(prev); next.set("open", id); return next; });
  }, [setSearchParams]);

  const closeDrawer = useCallback(() => {
    setOpenParticipantId(null);
    setSearchParams((prev) => { const next = new URLSearchParams(prev); next.delete("open"); return next; });
  }, [setSearchParams]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape" && selectedIds.size > 0 && !openParticipantId && !showAddModal) {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeTab !== "ALL") {
      list = list.filter((p: ParticipantRecord) =>
        p.enrollments.some((e: ParticipantEnrollment) => e.courseType === activeTab),
      );
    }
    ...
    if (completionFilter !== "ALL") {
      list = list.filter((p: ParticipantRecord) => {
        ...
        if (completionFilter === "미수료") return total === 0 || completed < total;
        return true;
      });
    }
    ...
    if (insuranceFilter !== "ALL") {
      list = list.filter((p: ParticipantRecord) => {
        ...
        if (insuranceFilter === "미가입") return p.employmentInsurance === "미가입";
        return true;
      });
    }
  ...
  const openParticipant = useMemo(
    () => (openParticipantId ? participants.find((p: ParticipantRecord) => p.id === openParticipantId) : null),
    [openParticipantId, participants],
  );

  const allFilteredSelected =
    filtered.length > 0 && filtered.every((p: ParticipantRecord) => selectedIds.has(p.id));
  ...
  const isFiltered =
    completionFilter !== "ALL" || insuranceFilter !== "ALL";

  const filtersActive = isFiltered || activeTab !== "ALL" || searchDebounced.trim() !== "";
        ...
        <table className="data-table participants-table">
          <thead>
            <tr>
              ...
              <th scope="col" style={{ width: 120, textAlign: "left" }}>이름</th>
              <th scope="col" style={{ textAlign: "left" }}>소속 기업</th>
              <th scope="col" style={{ width: 100, textAlign: "left" }}>직위</th>
              <th scope="col" className="col-phone participants-email-col" style={{ width: 140, textAlign: "left" }}>연락처</th>
              <th scope="col" className="col-email participants-email-col" style={{ textAlign: "left" }}>이메일</th>
              <th scope="col" style={{ width: 120, textAlign: "left" }}>수료 현황</th>
            </tr>
          </thead>
          <tbody>
                        {paginated.length === 0 ? (
              ...
              filtered.map((p: ParticipantRecord) => {
                ...
                return (
                  <tr
                    ...
                    tabIndex={0}
                    onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") openDrawer(p.id); }}
                  >
                    <td
                    ...
                    <td style={{ width: 100, color: "var(--color-text-secondary)", fontSize: 14 }}>
                      {p.position || "—"}
                    </td>
                    <td className="col-phone participants-email-col" style={{ width: 140, fontSize: 13 }}>{p.phone || "—"}</td>
                    <td className="col-email participants-email-col" style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>
                      {p.email || "—"}
                    </td>
                    <td style={{ width: 120 }}>
                        ...
                        >
                          <div
                            style={{
                              ...
                              background: variant === "green" ? "#10b981" : variant === "gray" ? "#94a3b8" : "#10b981",
                              borderRadius: 2,
      ...
      {selectedIds.size > 0 && (
        <div className="floating-action-bar">
          <div className="floating-action-inner">
            <p className="floating-action-count">{selectedIds.size}명 참여자 선택됨</p>
