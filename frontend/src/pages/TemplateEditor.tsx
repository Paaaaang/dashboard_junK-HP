...
import { 
  ...
  Strikethrough, Link, Highlighter, Type, Baseline
} from "lucide-react";
...
export function TemplateEditorPage() {
  ...
  const handleSelection = useCallback((e: any) => {
    ...
    // Use current mouse coordinates for precise tracking
    ...
    setTimeout(() => {
      ...
      if (start !== end) {
        setSelectionToolbar(prev => {
          ...
          return {
            x: Math.min(window.innerWidth - 300, Math.max(20, x)),
            y: Math.max(20, y),
            show: true
          };
        });
              ...
              <div className="px-1 py-3 mb-4 flex items-center justify-between border-b border-border/30">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[9px] font-bold text-tertiary">총 {templates.length}개의 서식</span>
                  </div>
                </div>
                ...
                {templates.map((template) => (
                  <button key={template.id} type="button" className={template.id === activeTemplateId ? "template-item template-item-active" : "template-item"} onClick={() => setActiveTemplateId(template.id)}>
                    <div className="flex justify-between items-start w-full">
                      ...
                      <span className={`shrink-0 text-[9px] font-black px-1.5 py-0.5 rounded-md ${template.audience === "INSURED" ? "bg-brand-primary/10 text-brand-primary" : template.audience === "UNINSURED" ? "bg-warning/10 text-warning" : "bg-surface-subtle text-tertiary"}`}>{template.audience === "INSURED" ? "가입" : template.audience === "UNINSURED" ? "미가입" : "전체"}</span>
                    </div>
                    <p className="text-[11px] text-tertiary font-medium line-clamp-1 opacity-70 mb-1">{template.subject || "제목 없음"}</p>
                    <div className="flex items-center justify-between mt-auto">
                      <div className="flex items-center gap-2">{template.attachments && template.attachments.length > 0 && <div className="flex items-center gap-1 bg-surface-subtle px-1.5 py-0.5 rounded-md border border-border/50"><Paperclip size={10} className="text-tertiary" /><span className="text-[9px] text-tertiary font-bold">{template.attachments.length}</span></div>}</div>
                      <span className="text-[9px] font-black text-disabled uppercase tracking-wider italic">Saved</span>
                    </div>
                  </button>
                ))}
              </div>
                ...
                <div className="overflow-hidden bg-surface">
                  <div className="px-6 py-2.5 bg-surface-subtle/50 border-b border-border flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] font-black text-tertiary uppercase tracking-wider">대상</span>
                  ...
                  <div className="px-6 py-5 bg-white space-y-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 space-y-1">
                        <div className="flex items-center gap-2"><span className="text-[10px] font-black text-disabled uppercase tracking-widest">Subject</span><div className="flex-1 h-px bg-border/20" /></div>
                ...
                <div className="editor-main-grid">
                  <div className="editor-section">
                    <div className="editor-section-header">
                      ...
                      <div className="flex items-center gap-2 group relative z-50"><HelpCircle size={14} className="text-tertiary cursor-help" /><div className="absolute bottom-full right-0 mb-2 w-48 p-2 bg-brand-dark text-white text-[10px] rounded-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-[100] shadow-xl">텍스트를 드래그하여 서식을 지정하거나, 상단 버튼으로 변수를 삽입하세요.</div></div>
                    </div>
                    <div className="flex flex-wrap gap-2 p-3 mb-5 bg-surface-subtle/50 rounded-2xl border border-border/50 relative z-30">
                      <div className="flex items-center gap-2 mr-2 pr-3 border-r border-border/50"><span className="text-[10px] font-black text-tertiary uppercase tracking-widest">Variables</span></div>
                      {templateVariableMetadata.map((v) => (
                        <button key={v.key} type="button" onMouseDown={(e) => e.preventDefault()} className="group relative px-3 py-1.5 bg-white border border-border hover:border-brand-primary hover:text-brand-primary text-[11px] font-black rounded-xl transition-all shadow-sm active:scale-95" onClick={() => insertVariable(v.key)}>{v.label}<div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-[180px] p-2 bg-brand-dark text-white text-[10px] font-medium rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-[100] shadow-xl">{v.description}<div className="absolute top-full left-1/2 -translate-x-1/2 border-8 border-transparent border-t-brand-dark" /></div></button>
                      ))}
                    </div>
                              ...
                              <div className="relative">
                                ...
                                {activeMenu === 'font' && (
                                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-36 bg-[#0f172a] border border-white/20 rounded-xl py-1.5 shadow-2xl animate-in fade-in slide-in-from-bottom-2">
                                    {fonts.map(f => <button key={f.value} onMouseDown={(e) => e.preventDefault()} onClick={() => applyFormat('font', 'face', f.value)} className="w-full text-left px-3 py-1.5 text-[10px] font-bold text-white/80 hover:text-white hover:bg-white/10 transition-all">{f.name}</button>)}
                              ...
                              <div className="relative">
                                ...
                                {activeMenu === 'size' && (
                                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-20 bg-[#0f172a] border border-white/20 rounded-xl py-1.5 shadow-2xl grid grid-cols-2 gap-0.5 px-1.5 animate-in fade-in slide-in-from-bottom-2">
                                    {fontSizes.map(s => <button key={s.value} onMouseDown={(e) => e.preventDefault()} onClick={() => applyFormat('size', 'value', s.value)} className="text-center py-1.5 text-[9px] font-black text-white/80 hover:text-white hover:bg-white/10 transition-all rounded-md">{s.label}</button>)}
                                    <div className="absolute top-full left-1/2 -translate-x-1/2 border-6 border-transparent border-t-[#0f172a]" />
                                  </div>
                                )}
                              </div>
                  ...
                  <div className="editor-section bg-surface-subtle/20">
                    <div className="editor-section-header">
                      ...
                      <div className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-success animate-pulse"></span><span className="text-[10px] font-bold text-success">실시간 렌더링</span></div>
                    </div>
                    <div className="flex-1 flex flex-col min-h-0 bg-white rounded-3xl border border-border/50 shadow-xl overflow-hidden mb-5">
                      <div className="px-8 py-6 border-b border-border/30 space-y-2">
                        <div className="flex items-center justify-between"><span className="text-[10px] font-black text-disabled uppercase tracking-widest italic">To: {mockData.name} ({mockData.companyName})</span><span className="text-[10px] font-bold text-tertiary">{toDotDate(new Date().toISOString())}</span></div>
                      ...
                      {(draftTemplate.attachments?.length || 0) > 0 && (
                        <div className="px-8 py-5 border-t border-border/30 bg-surface-subtle/30">
                          <p className="text-[9px] font-black text-tertiary uppercase tracking-widest mb-3">첨부파일 ({draftTemplate.attachments?.length})</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-border/30">
                  {logs.length === 0 ? (
                    <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-disabled italic">최근 발송 이력이 없습니다.</td></tr>
                  ) : logs.map(log => (
                    <tr key={log.id} className="hover:bg-brand-primary/5 transition-colors group">
                      <td className="px-6 py-4"><div className="flex items-center gap-2 text-xs font-bold text-secondary"><Clock size={12} className="text-disabled" />{log.sentAt ? `${toDotDate(log.sentAt)} ${new Date(log.sentAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : toDotDate(log.createdAt)}</div></td>
                      ...
                      <td className="px-6 py-4"><span className={`px-2 py-0.5 rounded text-[10px] font-black ${log.status === "sent" ? "bg-success/10 text-success" : log.status === "failed" ? "bg-error/10 text-error" : "bg-warning/10 text-warning"}`}>{log.status === "sent" ? "성공" : log.status === "failed" ? "실패" : "대기"}</span></td>
                      <td className="px-6 py-4">{log.errorMessage ? (<div className="flex items-center gap-1.5 text-[10px] font-bold text-error"><AlertCircle size={12} />{log.errorMessage}</div>) : (<span className="text-[10px] font-bold text-tertiary">{log.status === "sent" ? "정상 발송됨" : "-"}</span>)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
