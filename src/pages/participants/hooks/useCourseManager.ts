...
export function useCourseManager() {
  ...
  const isManagerGroupModified = useMemo(() => {
    if (!managerSelectedGroupId) {
      // For new group, modified if name is not empty or any detail exists
      return managerGroupForm.name.trim() !== "" || managerGroupForm.details.length > 0;
    }
    const original = courseGroups.find((g) => g.id === managerSelectedGroupId);
    if (!original) return false;

    // Compare only relevant fields to avoid metadata mismatches
    const originalComparable = {
      name: original.name,
      audiences: [...original.audiences].sort(),
    ...
    const currentComparable = {
      name: managerGroupForm.name,
      audiences: [...managerGroupForm.audiences].sort(),
      details: managerGroupForm.details.map(d => ({
        name: d.name,
        startDate: d.startDate,
        endDate: d.endDate,
        totalHours: d.totalHours,
    ...
    return JSON.stringify(originalComparable) !== JSON.stringify(currentComparable);
