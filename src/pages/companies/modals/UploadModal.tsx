      ...
      <div className="bg-surface rounded-2xl shadow-xl flex flex-col w-full max-w-4xl h-[85vh] overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
        <div className="px-8 py-6 border-b border-border flex justify-between items-center bg-surface sticky top-0 z-10">
          <div className="flex items-center gap-4">
            <h3 className="text-xl font-bold text-primary tracking-tight">데이터 일괄 등록</h3>
                ...
                <>
                  ...
                  <div className="p-6 bg-surface-subtle rounded-xl border border-border/40">
                    ...
                    <ul className="text-sm text-secondary space-y-2 leading-relaxed list-disc list-inside font-medium opacity-90">
                      ...
                      <li>데이터가 많을 경우(1,000건 이상) 분석에 수 초가 소요될 수 있습니다.</li>
                      <li>다음 단계에서 엑셀의 열과 시스템 필드를 자유롭게 연결할 수 있습니다.</li>
                    </ul>
                  </div>
                </>
              )}
            ...
            <div className="p-8 space-y-6 flex-1 flex flex-col min-h-0">
              <div className="flex justify-between items-end px-2">
                <div>
                  ...
                  <p className="text-sm text-tertiary mt-1 font-medium">엑셀의 각 항목을 시스템의 데이터 필드와 연결해 주세요.</p>
                </div>
              ...
              <div className="flex-1 min-h-0 border border-border rounded-xl overflow-hidden bg-surface shadow-sm">
                <div className="h-full overflow-y-auto custom-scrollbar">
                  <div className="grid grid-cols-[1fr_1fr] gap-px bg-border/40 sticky top-0 z-20">
                    ...
                    <div className="bg-surface-subtle px-8 py-3 text-[10px] font-black text-tertiary uppercase tracking-widest">시스템 필드 (저장 대상)</div>
                  </div>
                  <div className="divide-y divide-border/40">
                    {Object.keys(rawRows[0]).map((colName) => {
                      const currentMapping = columnMapping[colName] ?? "__skip__";
                      
                      return (
                        <div key={colName} className="grid grid-cols-[1fr_1fr] group hover:bg-surface-subtle/20 transition-colors">
                          {/* Excel Column Area */}
                          <div className="p-6 px-8 flex items-center gap-4 bg-surface-subtle/10 border-r border-border/40">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[11px] text-tertiary truncate mt-1.5 flex items-center gap-1.5 font-medium">
                                ...
                                {String(rawRows[0][colName] ?? "") || "(비어 있음)"}
                              </p>
                            </div>
                          </div>
              ...
              <div className="p-6 bg-surface-subtle border border-border rounded-xl flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-secondary font-medium">총 {uploadPreview.length}건의 데이터를 성공적으로 로드했습니다. 아래 내용을 최종 확인해 주세요.</p>
                  </div>
                </div>
              </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="sticky top-0 z-10">
                      <tr className="bg-surface-subtle border-b border-border shadow-sm">
                        {systemFields.filter(f => f.key !== "__skip__" && !["participationCount", "participationList", "enrollmentCount", "completionCount", "enrollmentSummary"].includes(f.key)).map(f => (
                          <th key={f.key} className="px-6 py-4 text-[10px] font-black text-tertiary uppercase tracking-widest bg-surface-subtle">{f.label}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border/30">
                      {uploadPreview.map((item, idx) => (
                        <tr key={idx} className="hover:bg-surface-subtle/30 transition-colors">
                          {systemFields.filter(f => f.key !== "__skip__" && !["participationCount", "participationList", "enrollmentCount", "completionCount", "enrollmentSummary"].includes(f.key)).map(f => (
                            <td key={f.key} className="px-6 py-4 text-sm font-medium text-secondary truncate max-w-[200px] tabular-nums font-mono">
