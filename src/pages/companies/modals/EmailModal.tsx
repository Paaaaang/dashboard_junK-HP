...
import { X, Send, Mail, Check, AlertCircle, ChevronRight, Users, Loader2, Search, Calendar as CalendarIcon, BookOpen } from "lucide-react";
import { useTemplateStore, useCourseStore, useToastStore, useCompanyStore } from "@/stores";
...
export function EmailModal({
  ...
  const { sendBatch, activeJob, subscribeToTemplates, setActiveJob } = useTemplateStore();
  ...
  }, [fetchCourseGroups, subscribeToTemplates, setActiveJob]);
      ...
      <div className="bg-surface rounded-3xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <header className="px-6 py-5 border-b border-border/50 flex items-center justify-between bg-surface relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-primary tracking-tight">기업 이메일 발송</h3>
                ...
                <div className="p-6 bg-surface border border-border/50 rounded-2xl shadow-sm space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="text-[10px] font-black text-tertiary uppercase mb-1">수신 기업 수</p>
                      <p className="text-sm font-bold text-brand-primary">{emailRecipientIds.length}개</p>
                    </div>
                  </div>
