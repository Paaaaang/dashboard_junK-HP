                  ...
                  <button key={template.id} type="button" className={template.id === activeTemplateId ? "template-item template-item-active" : "template-item"} onClick={() => setActiveTemplateId(template.id)}>
                    <div className="flex flex-col items-center gap-1 w-full">
                      <div className="flex items-center gap-2 justify-center w-full">
                        <p className="template-item-title truncate">{template.name}</p>
                      ...
                      <p className="text-[11px] text-tertiary font-medium line-clamp-1 opacity-70">{template.subject || "제목 없음"}</p>
                    </div>
                    <div className="flex items-center justify-center gap-3 mt-1 w-full opacity-60">
                      {template.attachments && template.attachments.length > 0 && (
                        <div className="flex items-center gap-1 bg-surface-subtle px-1.5 py-0.5 rounded-md border border-border/50">
                          <Paperclip size={10} className="text-tertiary" />
                          <span className="text-[10px] text-tertiary font-bold">{template.attachments.length}</span>
                        </div>
                      )}
                      <span className="text-[10px] font-black text-disabled uppercase tracking-wider italic">Saved</span>
                    </div>
                  </button>
                ))}
              </div>
