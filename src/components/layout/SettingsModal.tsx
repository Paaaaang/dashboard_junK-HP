...
export function SettingsModal({ onClose }: SettingsModalProps) {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        if (settings.naver_smtp) {
          setNaverEmail(settings.naver_smtp.email || "");
          setNaverPassword(settings.naver_smtp.password || "");
          setNaverHost(settings.naver_smtp.host || "smtp.naver.com");
          setSmtpPort(settings.naver_smtp.port || "465");
        }
      } catch (err) {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      setTestResult("error");
      const errorMessage = err.message || "SMTP 연결 실패. 정보를 확인해주세요.";
